    ...
    try:
        ...
        if fetch_all:
            ...
            if total_count > result_count:
                display_message += f" (showing {result_count} of {total_count}, use fetch_all=True to get all)"
...
async def get_page_impl(
    ...
    # Build path with optional version - using v1 API endpoint
    path = f"content/{id}"
    if version:
    ...
    try:
        ...
        return {
            "page": page,
            "message": f"Page {page.get('title', id)} retrieved successfully"
        }
    except Exception as e:
...
async def update_page_impl(
    ...
    try:
        ...
        return {
            "page": page,
            "message": f"Page {page.get('title', id)} updated successfully"
        }
    except Exception as e:
...
async def delete_page_impl(id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "id": id, 
            "deleted": True,
            "message": f"Page with ID {id} deleted successfully"
        }
    except Exception as e:
