...
def get_client() -> "ConfluenceApiClient":
    ...
    return ConfluenceApiClient(
        ...
        user_email=settings.JIRA_API_USER,
    )
        ...
        async def wrapper(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    logger.warning(
                        f"Connection error, retrying ({retries}/{max_retries}): {str(e)}"
                    )
...
class ConfluenceApiClient:
    ...
    def __init__(
        ...
        self.api_key = api_key or settings.JIRA_API_TOKEN
        self.user_email = user_email or settings.JIRA_API_USER

        # Validate required configuration
        if not self.base_url:
            raise ValueError(
                "JIRA_BASE_URL is required. Please set the JIRA_BASE_URL environment variable "
                "to your Confluence instance URL (e.g., https://your-domain.atlassian.net/wiki)"
            )

        if not self.api_key:
            raise ValueError(
                "JIRA_API_TOKEN is required. Please set the JIRA_API_TOKEN environment variable "
                "to your Confluence API token"
            )

        if not self.user_email:
            raise ValueError(
                "JIRA_API_USER is required. Please set the JIRA_API_USER environment variable "
        ...
        if not self.base_url.startswith(("http://", "https://")):
            raise ValueError(
                f"JIRA_BASE_URL must start with http:// or https://. Got: {self.base_url}"
            )
        ...
        self.client = httpx.AsyncClient(headers=self.headers, timeout=timeout, limits=self.limits)
    ...
    def _process_response(self, response: httpx.Response) -> Dict[str, Any]:
        ...
        if response.status_code >= 400:
            ...
            logger.error(
                f"API error ({response.status_code}): {error_message} - {detailed_message}"
            )
    ...
    async def fetch_all_pages(
        self, path: str, params: Optional[Dict[str, Any]] = None, max_pages: Optional[int] = None
    ) -> List[Dict[str, Any]]:
